...
Custom domain **www.rajasthanpickleball.com** (Vercel domain + DNS — user action). **HD RPA logo** — source `nihal-logo.jpeg` is 1172×1123 but WhatsApp-compressed; the *pixelation* bug (LogoMark hardcoded `sizes="40px"` while the preloader renders at 144px) is FIXED, so only a cleaner source file would improve it further. **HD `poster-legacy.png`** — Bhavya sent one in chat 2026-07-26 but pasted chat images can't be written to disk; ask them to save it to `~/Downloads`. **Jersey link** (About). IPA Nationals 2025 / Pickle Up footage. Instagram-feed embed on landing.
